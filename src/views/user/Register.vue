    ...
    <a-form ref="formRegister" :form="form" id="formRegister">
      <a-form-item>
        <a-input
          ...
          type="text"
          :placeholder="$t('姓名')"
          v-decorator="['name', {rules: [{ required: true, type: 'name' }], validateTrigger: ['change', 'blur']}]"
        ></a-input>
      ...
      <a-form-item>
        <a-input
          ...
          type="text"
          :placeholder="$t('学号')"
          v-decorator="['uid', {rules: [{ required: true, type: 'uid' }], validateTrigger: ['change', 'blur']}]"
        ></a-input>
      </a-form-item>
      <a-popover
        placement="rightTop"
        ...
        <a-form-item>
          <a-input-password
            ...
            v-decorator="['password', {rules: [{ required: true, message: $t('user.password.required') }, { validator: this.handlePasswordLevel }], validateTrigger: ['blur']}]"
          ></a-input-password>
        </a-form-item>
      </a-popover>
...
import { getSmsCaptcha, register, verification } from '@/api/login'
   ...
   /* handleSubmit () {
    ...
    handleSubmit () {
      ...
      if (username == null || uid == null) {
        ...
        registerParam.uid = uid
        registerParam.username = username
        registerParam.password = password
        verification(registerParam).then((res) => this.verification(res))
      }
    },
    verification (res) {
      if (res.data === 'false') {
        this.$message.warn('您已注册有问题请联系管理员')
      } else {
        const username = this.form.getFieldValue('name')
        const uid = this.form.getFieldValue('uid')
        const password = this.form.getFieldValue('password')
        ...
        registerParam.username = username
        registerParam.password = password
        register(registerParam).then((response) => (this.$router.push({ name: 'registerResult', params: { response } })))
      }
    },
    getCaptcha (e) {
      e.preventDefault()
